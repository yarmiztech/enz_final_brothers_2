...
class SalesReturn(models.Model):
    ...
    def compute_special_vali_button(self):
        for each in self:
            each.special_vali_button = False
...
class AccountMove(models.Model):
    ...
    manual_invoice_no = fields.Char(string="Manual Invoice Number")
...
class SaleEstimate(models.Model):
    ...
    def default_c_date(self):
        return datetime.today().date()

    c_date = fields.Date(string='Creation Date', index=True, default=default_c_date,
                         help="Date on which sales order is created.")

    # c_date = fields.Date(string='Creation Date', index=True,default=fields.Date.today(),
    #                               help="Date on which sales order is created.")